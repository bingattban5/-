...
class DownloadWorker @AssistedInject constructor(
    ...
    private suspend fun downloadVideoOnly(
        ...
        try {
            // ✅ الإصلاح: استخدام try/catch بدلاً من .catch
            try {
                ytDlpEngine.downloadVideo(url, formatId, outputPath).collect { progress ->
        ...
        try {
            ...
            val subtitleResult = when (subtitleMethod) {
                ...
                SubtitleMethod.WHISPER_GENERATED -> {
                    ...
                    // ✅ الإصلاح: استخدام try/catch بدلاً من .catch
                    try {
                        ffmpegEngine.extractAudio(videoFile.absolutePath, audioPath, "wav").collect { progress ->
    ...
    private suspend fun downloadSubtitleOnly(
        ...
        val subtitleResult = when (subtitleMethod) {
            ...
            SubtitleMethod.WHISPER_GENERATED -> {
                ...
                // ✅ الإصلاح: استخدام try/catch بدلاً من .catch
                try {
                    ytDlpEngine.downloadVideo(url, "bestaudio", tempVideoPath).collect { progress ->
                        updateNotification(progress.progress, "جاري التحميل: ${progress.progress}%")
                    }
                } catch (e: Exception) {
                ...
                if (downloadError != null) {
                    ...
                    // ✅ الإصلاح: استخدام try/catch بدلاً من .catch
                    try {
                        ffmpegEngine.extractAudio(tempVideoPath, audioPath, "wav").collect { progress ->
                            updateNotification(progress.progress, "استخراج الصوت: ${progress.progress}%")
                        }
                    } catch (e: Exception) {
                        extractError = e.message
